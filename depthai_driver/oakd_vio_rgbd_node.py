...
class OakdVioRgbdNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("rgbd_publish_every_n", 1)
    ...
    def poll(self):
        ...
        if self.latest_left is not None and self.latest_right is not None:
            ...
            # -------------------------
            publish_rgbd = (
                self.vio_frame_count % self.rgbd_publish_every_n == 0
            )
            if publish_rgbd:
                self.publish_latest_rgbd(stamp)
    ...
    def publish_latest_rgbd(self, stamp):
        """
